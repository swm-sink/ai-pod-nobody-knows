...
**Agents:** `.claude/agents/simplified/` - AI workers that do specific tasks  
**Commands:** `.claude/commands/` - Workflows you can run
**Config:** `.claude/config/` - Settings and API keys
**Context:** `.claude/context/simple/` - Basic documentation

## 🎙️ Simple Production Workflow

**How it works:**
1. **Research**: Find information about topic
2. **Script**: Write episode script  
3. **Audio**: Generate speech with ElevenLabs
4. **Quality**: Check it meets standards

**Cost:** Target $3-5 per episode  
**Time:** 15-30 minutes
**Output:** 28-minute podcast episode

## 🌐 COMPLETE HIERARCHICAL MEMORY SYSTEM

<HIERARCHICAL_MEMORY_INTEGRATION research="comprehensive" implementation="enterprise-grade">
**Multi-Tier Memory Architecture:** Complete system with intelligent selective loading, context prediction, and automated memory optimization across all system domains.

### **Tier 3: Domain Memory Systems**
```yaml
infrastructure_domain:
  claude_system: "@.claude/CLAUDE.md"
  triggers: "infrastructure|system|core|foundation"
  loads_with: ["agents", "commands", "context", "config", "hooks"]
  
content_domain:
  episodes: "@episodes/CLAUDE.md"  
  sessions: "@sessions/CLAUDE.md"
  projects: "@projects/nobody-knows/CLAUDE.md"
  triggers: "episode|content|production|publishing"
  
development_domain:
  source_code: "@src/CLAUDE.md"
  documentation: "@docs/CLAUDE.md" 
  testing: "@tests/CLAUDE.md"
  triggers: "development|coding|documentation|testing"
  
operations_domain:
  scripts: "@scripts/CLAUDE.md"
  build: "@build/CLAUDE.md"
  dashboard: "@dashboard/CLAUDE.md"
  triggers: "operations|deployment|monitoring|utilities"
```

### **Tier 4: Component Memory Systems**
```yaml
claude_infrastructure:
  ...
  hooks: "@.claude/hooks/CLAUDE.md"
  infrastructure: "@.claude/infrastructure/CLAUDE.md"
  
content_components:
  episode_templates: "@episodes/templates/"
  completed_episodes: "@episodes/completed/"
  active_sessions: "@sessions/[current]/"
  
development_components:
  agent_files: "@.claude/agents/simplified/"
  command_files: "@.claude/commands/simplified/"
  context_files: "@.claude/context/simplified/"
```

### **Advanced Selective Loading Intelligence**
```yaml
command_triggered_loading:
  "/init":
    loads: ["@.claude/CLAUDE.md", "@sessions/CLAUDE.md"]
    purpose: "Infrastructure + session management"
    
  "/podcast-workflow":
    loads: ["@episodes/CLAUDE.md", "@.claude/agents/CLAUDE.md", "@.claude/commands/CLAUDE.md"]
    purpose: "Complete production pipeline context"
    
  "/research-workflow":
    loads: ["@.claude/agents/CLAUDE.md", "@sessions/CLAUDE.md"]
    purpose: "Research-focused agent and session context"
    
  "/audio-workflow":
    loads: ["@.claude/config/CLAUDE.md", "@.claude/agents/CLAUDE.md"]
    purpose: "Audio production configuration and agent context"

agent_triggered_loading:
  "researcher agent":
    loads: ["@.claude/agents/CLAUDE.md", "@.claude/config/CLAUDE.md"]
    purpose: "Agent architecture + MCP configuration"
    
  "writer agent":
    loads: ["@.claude/agents/CLAUDE.md", "@episodes/CLAUDE.md"]
    purpose: "Agent context + episode management"
    
  "audio-producer agent":
    loads: ["@.claude/config/CLAUDE.md", "@.claude/infrastructure/CLAUDE.md"]
    purpose: "Voice configuration + infrastructure services"

task_triggered_loading:
  "development tasks":
    loads: ["@src/CLAUDE.md", "@tests/CLAUDE.md", "@docs/CLAUDE.md"]
    purpose: "Development-focused context stack"
    
  "operations tasks":
    loads: ["@scripts/CLAUDE.md", "@build/CLAUDE.md", "@dashboard/CLAUDE.md"]
    purpose: "Operations-focused context stack"
    
  "configuration tasks":
    loads: ["@.claude/config/CLAUDE.md", "@.claude/hooks/CLAUDE.md"]
    purpose: "Configuration and automation context"
```

### **Memory Optimization Intelligence**
```yaml
optimization_patterns:
  predictive_loading:
    research_workflow: "Pre-load agent and session contexts"
    production_workflow: "Pre-load quality and episode contexts"
    development_work: "Pre-load source and testing contexts"
    
  context_caching:
    session_duration: "Cache all loaded contexts for entire session"
    cross_command: "Reuse contexts across related commands"
    intelligent_eviction: "Remove unused contexts when memory pressure"
    
  token_efficiency:
    lazy_loading: "Load contexts only when actually accessed"
    smart_unloading: "Remove contexts when no longer needed"
    cascade_optimization: "Load dependent contexts intelligently"
```

**Technical:** Complete hierarchical memory system implementing enterprise-grade context management with intelligent selective loading, predictive optimization, and automated memory efficiency for scalable AI orchestration.

**Simple:** Like having a brilliant assistant who always knows exactly what information you need, brings it instantly, and keeps everything perfectly organized without you having to think about it.

**Connection:** This teaches advanced system architecture, memory management, and intelligent automation patterns that are fundamental to building sophisticated, efficient AI applications that scale gracefully.
...
**Purpose:** Initialize project memory and session tracking.

**What it does:**
- Loads this CLAUDE.md file  
- Sets up cost tracking
- Initializes session for episode creation

**Usage:** Just run `/init` before starting work.
</SYSTEM_COMMANDS>
